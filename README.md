...
### 🚨 訊號偵測 (PRR / ROR)
- 以 FAERS 全庫為背景自動建立 2×2 列聯表
- 計算 PRR、ROR（含 95% 信賴區間）與 Yates 校正卡方
- 依 EMA 標準（a ≥ 3、PRR ≥ 2、χ² ≥ 4）自動判定訊號
- ROR 森林圖視覺化比較多藥品

### 🌍 全球地理分析
- 世界地圖 (choropleth) 視覺化：顯示各國 ADR 通報分布
- Top 15 通報國家排名
...
### 🔎 查詢範圍控制
- 通報期間過濾（全部 / 近 3 / 5 / 10 年）
- 僅嚴重案件 (serious=1) 過濾
- openFDA API Key 支援（提高查詢額度至 120,000 次/日）

### 📤 資料匯出
- 下載 Case-level 詳細資料（分頁抓取，最多 500 筆）
- 分析摘要、訊號偵測表 CSV；完整 Excel（摘要 + 訊號 + 案件）
- 包含：安全報告 ID、患者資訊、劑量、給藥途徑等
